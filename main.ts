...
async function main() {
  try {
    ...
    const followers = new Set(
      (followersList as {login: string}[]).map(u => u.login),
    );
    const following = new Set(
      (followingList as {login: string}[]).map(u => u.login),
    );

    console.log(
      `Found ${followers.size} followers and ${following.size} following.`,
    );
    ...
    for (const follower of [...followers].filter(f => !following.has(f))) {
      ...
      if (response.status >= 200 && response.status < 300) {
        console.log(
          `Followed ${follower} unless the user's profile was private.`,
        );
      }
    }

    for (const followedUser of [...following].filter(f => !followers.has(f))) {
      const response = await octokit.rest.users.unfollow({
        username: followedUser,
      });
      if (response.status >= 200 && response.status < 300) {
        console.log(
          `Unfollowed ${followedUser} unless the user's profile was private.`,
        );
      }
    }
  } catch (error) {
